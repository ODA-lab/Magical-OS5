...
//	{ 0x8086, 0x7010, 1 }, // PIIX3 IDE Interface (Triton II)
	{ 0x8086, 0x7111, 1 }, // Intel(R) 82371AB/EB PCI Bus Master IDE Controller
};
...
static bool sector_rw_common(uint8_t cmd, int device, uint32_t sector)
{
	...
	outb_p(0x02, DEVICE_CONTROL_REGISTER);

	// Features register should be 0.
	outb_p(0x00, FEATURES_REGISTER);

	// Set Logical Sector.
	outb_p(sector & 0xff, SECTOR_NUMBER_REGISTER);
	outb_p((sector >> 8) & 0xff, CYLINDER_LOW_REGISTER);
	outb_p((sector >> 16) & 0xff, CYLINDER_HIGH_REGISTER);
	outb_p(((sector >> 24) & 0x1f) | 0x40, DEVICE_HEAD_REGISTER);
	outb_p(1, SECTOR_COUNT_REGISTER);
